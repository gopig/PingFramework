#!/data/soft/php/bin/php
<?php
/**
 * 压力测试工具 用法
 * php bench.php -c 100 -n 10000 -s tcp://127.0.0.1:9501 -f long_tcp
 * php bench.php -c1 -n1 -fshort_tcp -s tcp://112.126.69.14:9501
 */
//关闭错误输出
//error_reporting(0);
$shortopts = "c:";
...
$bc->send_data = getSendData('role.getRole');
$bc->read_len = 65536;
...
{
    $fp = new swoole_client(SWOOLE_SOCK_TCP, SWOOLE_SOCK_SYNC);
    if(!$fp->connect($bc->server_config['host'], $bc->server_config['port'], 30))
    {
        error:
        echo "Error: [{$fp->errCode}]\n";
        return false;
    }
    else
    {
